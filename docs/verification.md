...
| AI analysis             | Partially verified | Strict input/output schemas, frame-evidence prompts, isolated IPC, deterministic provider behavior, and the complete intent interview passed `pnpm check` and `pnpm test:e2e` on 2026-07-14. `pnpm test:live` reached OpenAI but failed with `429 exceeded current quota`; no live analysis result has passed yet. |
| Audio extraction        | Planned            | The recorder currently captures screen video only. No audio capture, extraction, or transcription exists.                                                                                                                                                                                                          |
