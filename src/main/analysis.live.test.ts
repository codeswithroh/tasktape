...
describe('live OpenAI recording analysis', () => {
  it('returns a schema-bound learned workflow from a real TaskTape screenshot', async () => {
    ...
    const input: AnalyzeRecordingInput = {
      ...
      durationMs: 5_000,
      userIntent: 'Organize new assets using the folder structure shown in the recording.',
      frames: [
        {
          timestampMs: 2_500,
    ...
    expect(analysis.learnedWorkflow.steps.length).toBeGreaterThan(0)
  })
})
